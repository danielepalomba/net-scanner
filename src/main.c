...
void std_packet_handler(u_char *args, const struct pcap_pkthdr *header,
                        const u_char *packet) {
  ...
  if (ntohs(eth_header->ether_type) == ETHERTYPE_ARP) {
    ...
    if (device != NULL) {
      ...
      if (LEARNING_MODE) {
        ...
        logger_log(LOG_INFO, "New Device (Learning): %s [%s]", mac_str,
                   vendor_name);

        dm_add_device(net_manager, arp_header->arp_sha, arp_header->arp_spa,
                      true);
        ...
        logger_log(LOG_ERR, "ALERT: New Intruder! %s [%s] IP: %d.%d.%d.%d",
                   mac_str, vendor_name, arp_header->arp_spa[0],
                   arp_header->arp_spa[1], arp_header->arp_spa[2],
                   arp_header->arp_spa[3]);
...
void fast_packet_handler(u_char *args, const struct pcap_pkthdr *header,
                         const u_char *packet) {
  struct ether_header *eth_header = (struct ether_header *)packet;
...
void *consumer_routine(void *arg) {
  ...
  // Pre-configure server address structure
  memset(&serv_addr, 0, sizeof(serv_addr));
  serv_addr.sin_family = AF_INET;
  serv_addr.sin_port = htons(SERVER_PORT);
  if (inet_pton(AF_INET, SERVER_IP, &serv_addr.sin_addr) <= 0) {
    logger_log(LOG_ERR, "Invalid address/ Address not supported: %s",
               SERVER_IP);
    // If the address is invalid, we might as well exit the thread or keep
    // failing. For robustness, we'll just log it. The loop will try to connect
    // and fail, effectively disabling AI reporting but keeping the app running.
  }
  ...
  while (queue_pop(&packet_queue, &pkt)) {
    ...
    if (sock_fd < 0) {
      if ((sock_fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        logger_log(LOG_ERR, "Socket creation failed");
      } else {
        // Attempt connection
        if (connect(sock_fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) <
            0) {
          logger_log(LOG_WARN,
                     "Connection Failed to AI Engine (will retry next packet)");
          close(sock_fd);
          sock_fd = -1;
        } else {
    ...
    // Prepare Data
    snprintf(mac_str, sizeof(mac_str), "%02x:%02x:%02x:%02x:%02x:%02x",
             pkt.src_mac[0], pkt.src_mac[1], pkt.src_mac[2], pkt.src_mac[3],
    ...
    if (sock_fd >= 0) {
      ...
      snprintf(data_buffer, sizeof(data_buffer), "%ld,%s,%d.%d.%d.%d,%d\n",
               time(NULL), mac_str, pkt.src_ip[0], pkt.src_ip[1], pkt.src_ip[2],
               pkt.src_ip[3], is_trusted);
